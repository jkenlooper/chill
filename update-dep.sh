...
project_name="chill-dep"

# Use $PWD to avoid using 'realpath' since it isn't always available.
project_dir="$PWD"
test -e "$project_dir/$script_name" || (echo "ERROR: Should run $script_name from the project directory." 1>&2 && exit 1)
...
docker cp --quiet "$container_name:/home/dev/app/requirements-cli.txt" "$project_dir/requirements-cli.txt"
