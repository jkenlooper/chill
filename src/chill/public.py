...
class PageView(MethodView):
    ...
    def get(self, uri=''):
        ...
        if result:
            ...
            rendered = self.render_node(result[2], result[1], result[0])
            if rendered:
    ...
    def render_node(self, node_id, value, name):
        c = db.cursor()
        if value == None:
                    ...
                    if value and (len(value) >= 1):
                        ...
                        if 'node_id' in col_names:
                            ...
                            for v in value:
                                ...
                                list.append({v['name']: self.render_node(v['node_id'], v.get('value',None), v.get('name', None))})
                            value = list
            ...
            if linked_value:
                ...
                if len(linked_value) > 1:
                    ...
                    for v in linked_value:
                        current_app.logger.debug(v)
                        list.append({v[1]: self.render_node(v[2], None, v[1])})
                    linked_value = list
                else:
                    linked_value = self.render_node(linked_value[0][0]) #TODO
